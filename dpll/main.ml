...
let transform_tseitin exp =
  (** Параметры: аккумулятор, выражение
  ...
  let rec cnf e var_list acc =
    ...
    | Not e -> 
      ...
      p, p :: var_list1, [ -p ] :: acc1
  in
  let vars = exp |> get_all_vars_from_ast |> unique_els in
  let _, _, result = cnf exp vars [] in
  result
;;
...
let () =
  let cnf = transform_tseitin (Not (Operation (Or, Operation (And, Literal 1, Literal 2), Operation (And, Literal (-3), Literal (4))))) in
  print_acc cnf;
  ()
;;
